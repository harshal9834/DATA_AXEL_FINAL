...
import { Presentation, FileText, Download, Upload, Loader2, ChevronDown, Monitor, CheckCircle2 } from "lucide-react";
...
function DocsStudio() {
  ...
  const handleGenerateFromProject = async () => {
    ...
    toast.info("Generating documents...", { duration: 4000 });
  ...
  const downloadPPT = async () => {
    ...
    try {
      ...
      const addHeader = (slide: any, title: string, color = "10B981") => {
         slide.addShape(pres.ShapeType.rect, { x: 0, y: 0, w: "100%", h: 0.8, fill: { color: color } });
         slide.addText(title, { x: 0.5, y: 0.1, w: "90%", h: 0.6, fontSize: 24, color: "FFFFFF", bold: true });
      };

      // 1. Cover
      let s = pres.addSlide();
      s.background = { fill: "0F172A" };
      s.addText(docData.projectName || "Project", { x: 1, y: 2, w: 8, fontSize: 44, color: "FFFFFF", bold: true, align: "center" });
      s.addText(docData.summary || "", { x: 1, y: 3.2, w: 8, fontSize: 20, color: "94A3B8", align: "center" });
      s.addText("Generated by AI Document Studio", { x: 1, y: 5, w: 8, fontSize: 14, color: "3B82F6", bold: true, align: "center" });

      // 2. Problem
      s = pres.addSlide();
      addHeader(s, "THE PROBLEM", "3B82F6");
      ...
      docData.problem?.painPoints?.forEach((pt: string, i: number) => {
        s.addShape(pres.ShapeType.rect, { x: 0.5, y: 2.2 + (i * 0.8), w: 9, h: 0.6, fill: { color: "EFF6FF" }, line: { color: "BFDBFE", width: 1 } });
        s.addText(pt, { x: 0.8, y: 2.3 + (i * 0.8), w: 8.5, fontSize: 14, color: "1E3A8A" });
      });

      // 3. Solution
      s = pres.addSlide();
      addHeader(s, "THE SOLUTION", "10B981");
      ...
      docData.solution?.keyFeatures?.forEach((f: string, i: number) => {
        ...
        const y = 2.5 + Math.floor(i / 2) * 1;
        s.addShape(pres.ShapeType.roundRect, { x, y, w: 4.3, h: 0.8, fill: { color: "F0FDF4" }, line: { color: "BBF7D0", width: 1 } });
        s.addText(f, { x: x + 0.2, y: y + 0.2, w: 3.9, fontSize: 14, color: "166534" });
      });

      // 4. Stack
      s = pres.addSlide();
      addHeader(s, "TECHNOLOGY STACK", "8B5CF6");
      const cats = [
      ...
      cats.forEach((c, i) => {
        s.addShape(pres.ShapeType.rect, { x: 0.5, y: 1.5 + (i * 1.2), w: 9, h: 0.9, fill: { color: "F5F3FF" }, line: { color: "DDD6FE", width: 1 } });
        s.addText(c.name, { x: 0.8, y: 1.6 + (i * 1.2), w: 2, fontSize: 16, bold: true, color: "5B21B6" });
        s.addText(c.val || "", { x: 3, y: 1.6 + (i * 1.2), w: 6, fontSize: 14, color: "333333" });
      });

      // 5. Workflow
      s = pres.addSlide();
      addHeader(s, "SYSTEM WORKFLOW", "F59E0B");
      s.addText(docData.workflow?.description || "", { x: 0.5, y: 1.2, w: 9, fontSize: 16, color: "333333" });
      // Diagrams are not easily exported to PPTX in this lightweight version unless we use html-to-image. We leave a placeholder.
      s.addText("See web view for diagram.", { x: 0.5, y: 3, w: 9, fontSize: 14, color: "94A3B8", align: "center", italic: true });

      // 6. Architecture
      s = pres.addSlide();
      addHeader(s, "ARCHITECTURE", "EC4899");
      s.addText(docData.architecture?.description || "", { x: 0.5, y: 1.2, w: 9, fontSize: 16, color: "333333" });
      s.addText("See web view for diagram.", { x: 0.5, y: 3, w: 9, fontSize: 14, color: "94A3B8", align: "center", italic: true });
  ...
  return (
    <div className="flex h-full flex-col space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <button onClick={() => setShowPicker(!showPicker)} className="flex items-center gap-2 rounded-full bg-blue-500 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-600 transition-colors min-w-[200px] justify-between">
      ...
      {/* Tabs & Exports */}
      <div className="flex flex-col md:flex-row items-center justify-between border-b border-slate-200 pb-0">
        <div className="flex items-center gap-6 h-full">
          <button onClick={() => setActiveTab("PPT")} className={`px-2 py-3 text-sm font-bold border-b-2 transition-colors ${activeTab === "PPT" ? "border-blue-600 text-blue-600" : "border-transparent text-slate-500 hover:text-slate-800"}`}>
            Presentation View
          </button>
          <button onClick={() => setActiveTab("SRS")} className={`px-2 py-3 text-sm font-bold border-b-2 transition-colors ${activeTab === "SRS" ? "border-blue-600 text-blue-600" : "border-transparent text-slate-500 hover:text-slate-800"}`}>
            Document View
          </button>
        </div>
        <div className="flex gap-3 pb-2">
           <button onClick={downloadPPT} className="flex items-center gap-2 rounded-lg bg-orange-50 text-orange-600 px-4 py-2 text-xs font-bold hover:bg-orange-100 shadow-sm border border-orange-100 transition-colors">
             <Download className="h-3.5 w-3.5" /> Download PPTX
           </button>
           <button onClick={downloadDOCX} className="flex items-center gap-2 rounded-lg bg-blue-50 text-blue-600 px-4 py-2 text-xs font-bold hover:bg-blue-100 shadow-sm border border-blue-100 transition-colors">
             <Download className="h-3.5 w-3.5" /> Download DOCX
           </button>
           <button onClick={downloadPDF} className="flex items-center gap-2 rounded-lg bg-emerald-50 text-emerald-600 px-4 py-2 text-xs font-bold hover:bg-emerald-100 shadow-sm border border-emerald-100 transition-colors">
      ...
      {/* Main Content Area */}
      <div className="flex-1 bg-slate-50 rounded-xl overflow-y-auto relative p-6">
        ...
        {docData && activeTab === "PPT" && (
          <div className="max-w-4xl mx-auto space-y-12 pb-20">
            {/* Cover Card */}
            <div className="w-full aspect-[16/9] bg-slate-900 rounded-xl shadow-xl overflow-hidden relative flex flex-col justify-center items-center text-center p-12">
               <h1 className="text-5xl font-black text-white mb-6 leading-tight tracking-tight">{docData.projectName}</h1>
               <p className="text-xl text-slate-300 font-medium max-w-2xl">{docData.summary}</p>
               <div className="absolute bottom-8 text-blue-400 font-semibold text-sm">Generated by AI Document Studio</div>
            </div>

            {/* Problem Card */}
            <div className="w-full bg-white rounded-xl shadow-md overflow-hidden">
               <div className="bg-blue-50 px-8 py-5 border-b border-blue-100">
                 <h2 className="text-xl font-bold text-blue-900 tracking-wide">THE PROBLEM</h2>
               </div>
               <div className="p-8">
                 ...
                 <div className="space-y-4">
                   {docData.problem?.painPoints?.map((p:string, i:number) => (
                     <div key={i} className="flex items-center gap-4 bg-slate-50 p-4 rounded-lg border border-slate-100">
                        <div className="h-8 w-8 rounded-full bg-blue-100 text-blue-600 font-bold flex items-center justify-center shrink-0">{i+1}</div>
            ...
            <div className="w-full bg-white rounded-xl shadow-md overflow-hidden">
               <div className="bg-green-50 px-8 py-5 border-b border-green-100">
                 <h2 className="text-xl font-bold text-green-900 tracking-wide uppercase">The Solution</h2>
               </div>
               <div className="p-8">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   {docData.solution?.keyFeatures?.map((f:string, i:number) => (
                     <div key={i} className="flex items-center gap-4 bg-white p-4 rounded-xl border border-green-100 shadow-sm">
                        <div className="h-8 w-8 rounded-full bg-green-100 text-green-700 font-bold flex items-center justify-center shrink-0">{i+1}</div>
                        <span className="text-slate-800 font-semibold text-sm">{f}</span>
                     </div>
                   ))}
                 </div>
               </div>
            ...
            {/* Stack Card */}
            <div className="w-full bg-white rounded-xl shadow-md overflow-hidden">
               <div className="bg-purple-50 px-8 py-5 border-b border-purple-100">
            ...
            {/* Architecture Card */}
            <div className="w-full bg-white rounded-xl shadow-md overflow-hidden">
               <div className="bg-pink-50 px-8 py-5 border-b border-pink-100">
        ...
        {docData && activeTab === "SRS" && (
          <div className="max-w-4xl mx-auto flex justify-center">
            <div id="srs-document-container" className="w-full bg-white shadow-md p-12 text-slate-900 rounded-xl border border-slate-200">
...
with open(FILE_PATH, "w", encoding="utf-8") as f:
    f.write(CONTENT)
print("Updated app.docs.tsx to precisely match the provided vertically-scrolling web UI screenshot.")
