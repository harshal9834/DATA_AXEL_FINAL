...
NEW_DOWNLOAD_PPT = r"""  const downloadPPT = async () => {
    if (!docData) return;
...
match = re.search(r'const downloadPPT = async \(\) => \{[\s\S]*?pres\.writeFile\(\{[\s\S]*?toast\.success\("PPT Downloaded!"\);\n\s*\} catch \(e\) \{\n\s*toast\.error\("Failed to generate PPT"\);\n\s*console\.error\(e\);\n\s*\}\n\s*\};', content)

if match:
    new_content = content[:match.start()] + NEW_DOWNLOAD_PPT + content[match.end():]
    with open(FILE_PATH, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Updated PPT generation!")
else:
    print("Could not find downloadPPT block")
