...
import {
  LayoutDashboard, FolderKanban, Search, Network, Bell, Library,
  FileText, Sparkles, Settings, Sun, Command, LogOut, ChevronRight, Bot, Mic,
...
const nav: NavItem[] = [
  ...
  { to: "/app/architecture", label: "Smart Alerts",      icon: Bell },
        ...
        <header className="sticky top-0 z-30 border-b border-border/60 bg-background/70 backdrop-blur-xl">
          <div className="flex h-16 items-center gap-3 px-4 md:px-8">
            ...
            <div className="flex items-center gap-2 rounded-xl border border-border/60 bg-white/80 py-1 pl-1 pr-3">
              {user?.photoURL ? (
                <img src={user.photoURL} alt="Profile" className="h-7 w-7 rounded-lg object-cover" />
              ) : (
              ...
              <div className="hidden text-xs md:block">
                <div className="font-semibold leading-tight">{user?.displayName || 'User'}</div>
