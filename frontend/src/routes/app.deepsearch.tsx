...
import { Search, ExternalLink, Sparkles, Clock, CheckCircle2, Loader2, Play, Download, BrainCircuit, Lightbulb, FileText, Activity } from "lucide-react";
...
function DeepSearch() {
  ...
  const performSearch = async (category: string, topic: string, forceNew: boolean = false) => {
    ...
    // If we have cached data for this category in the current session, and it's not a forced new search, don't re-fetch
...
Do NOT wrap the response in markdown blocks. Return ONLY valid JSON.`;
  ...
  return (
    <div>
      <PageHeader title="DeepSearch" subtitle="AI-powered centralized research across domains." />
