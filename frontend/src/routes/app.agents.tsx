...
export const Route = createFileRoute("/app/agents")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "8 specialised AI agents that collaborate to turn ideas into implementation-ready projects." },
    ],
  }),
...
const agents: Agent[] = [
  ...
  },
  {
    ...
    currentTask: "Ready for new ideas",
  ...
  },
  {
    ...
    currentTask: "Ready for new designs",
  ...
  },
  {
    ...
    currentTask: "Ready for writing tasks",
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        fetch(`${BACKEND_URL}/api/workflows/dashboard`, {
          headers: {
    ...
    socket.on("workflow_log_added", (log) => {
      setLiveActivity(prev => [log, ...prev].slice(0, 20)); // keep last 20
    });
    ...
    socket.on("agent_started", (task) => {
      ...
      toast(`Agent started: ${task.agent}`);
    });
    ...
    socket.on("agent_completed", (task) => {
      setActiveTasks(prev => prev.map(t => t.id === task.id ? task : t));
      toast.success(`Agent completed: ${task.agent}`);
    });
    ...
    return () => {
      socketRef.current?.off("workflow_log_added");
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-8">
          <div className="max-w-xl">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              <button onClick={() => setShowWorkflowModal(true)} className="flex items-center gap-2 rounded-xl bg-gradient-brand px-4 py-2.5 text-sm font-semibold text-white shadow-glow">
...
// ... keeping other components mostly identical, just updating props for Dynamic ones
...
function Kanban({ tasks }: { tasks: any[] }) {
  const queued = tasks.filter(t => t.status === "WAITING");
  const running = tasks.filter(t => t.status === "RUNNING");
  const completed = tasks.filter(t => t.status === "COMPLETED");

  const cols = [
    { col: "Queued", icon: Clock, tasks: queued },
    { col: "Running", icon: Loader2, tasks: running },
    { col: "Completed", icon: CheckCircle2, tasks: completed },
  ];
          ...
          <div key={col.col} className="rounded-[18px] border border-border/60 bg-white/70 p-3 backdrop-blur">
            ...
            <div className="space-y-2">
              {col.tasks.map((t: any, i: number) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    <span className={`grid h-6 w-6 place-items-center rounded-lg bg-gradient-to-br ${t.color || 'from-slate-500 to-slate-600'} text-white`}>
                  ...
                  <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-muted">
                    <motion.div
                      initial={{ width: 0 }} animate={{ width: `${t.progress}%` }} transition={{ duration: 0.8 }}
                      className={`h-full rounded-full bg-gradient-to-r ${t.color || 'from-slate-500 to-slate-600'}`}
                    />
                  </div>
...
function ActivityFeed({ logs }: { logs: any[] }) {
  // Map icons from string to actual components if necessary, assuming backend sends names like "Search"
  const iconMap: Record<string, any> = { Search, Boxes, Calendar, FileText, Library, TrendingUp, CheckCircle2 };
  
  return (
    <div className="card-premium relative overflow-hidden p-5 min-h-[300px]">
          ...
          {logs.map((it: any, i: number) => {
            ...
            return (
              <motion.li
                ...
                className="relative"
              >
                <span className={`absolute -left-[22px] top-1 grid h-4 w-4 place-items-center rounded-full ${it.color || 'bg-slate-500'} text-white shadow-soft`}>
                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className="text-[10px] text-muted-foreground whitespace-nowrap">{it.time || "just now"}</span>
                </div>
              </motion.li>
            );
          })}
...
function Analytics({ data }: { data: any }) {
  // Use dummy data if `data` is not available yet
  const tasks = data?.tasks || [
    { d: "Mon", v: 12 }, { d: "Tue", v: 18 }, { d: "Wed", v: 22 },
    { d: "Thu", v: 16 }, { d: "Fri", v: 28 }, { d: "Sat", v: 20 }, { d: "Sun", v: 32 },
  ];
  const research = data?.research || tasks.map((t: any, i: number) => ({ d: t.d, v: 10 + i * 4 + (i % 2 ? 2 : 0) }));
  ...
  const totalCompleted = data?.totalCompleted || 148;
  const totalResearch = data?.totalResearch || 312;
  const totalProjects = data?.totalProjects || 35;
    ...
    <div className="grid gap-4 lg:grid-cols-3">
      <div className="card-premium p-5">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-xs font-semibold text-muted-foreground">Workflows completed</div>
            <div className="mt-1 text-2xl font-extrabold">{totalCompleted}</div>
          </div>
      ...
      <div className="card-premium p-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="mt-1 text-2xl font-extrabold">{totalResearch}</div>
          </div>
      ...
      <div className="card-premium p-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="mt-1 text-2xl font-extrabold">{totalProjects} <span className="text-xs font-normal text-muted-foreground">projects</span></div>
          </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={dist} dataKey="v" innerRadius={38} outerRadius={58} paddingAngle={2} stroke="none">
                  {dist.map((d: any) => <Cell key={d.name} fill={d.c} />)}
                </Pie>
                <Tooltip contentStyle={{ border: "1px solid #e5e7eb", borderRadius: 12, fontSize: 11 }} />
              </PieChart>
            </ResponsiveContainer>
